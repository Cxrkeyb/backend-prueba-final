...
const readPrivateKey = async () => {
  if (!existsSync(`./src/keys/${config.web.appKey}.pem`)) {
    ...
    const keyWritePromises = [
      ...
      writeFileSync(
        `./src/keys/${config.web.appKey}.pem`,
        await jose.exportPKCS8(keyPair.privateKey)
      ),
      writeFileSync(
        `./src/keys/${config.web.appKey}pub.pem`,
  ...
  privateKey = await jose.importPKCS8(
    readFileSync(`./src/keys/${config.web.appKey}.pem`, "utf-8"),
    keyAlgorithm
  );
  publicKey = await jose.importSPKI(
    readFileSync(`./src/keys/${config.web.appKey}pub.pem`, "utf-8"),
    keyAlgorithm
  );
};
...
const generateJwt = async (payload: string, exp: string) => {
  const jwt = await new jose.EncryptJWT(JSON.parse(payload))
